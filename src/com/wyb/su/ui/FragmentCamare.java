...
import com.wyb.su.utils.RecorderParameters;
import com.wyb.su.utils.SavedFrames;
import com.wyb.su.utils.Util;
import com.wyb.su.views.NewFFmpegFrameRecorder;
import com.wyb.su.views.ProgressView;
import com.wyb.su.views.ProgressView.State;
...
public class FragmentCamare extends Fragment implements OnClickListener, OnTouchListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		//setContentView(R.layout.camare_fragment_layout);
	...
	{
		stateImageView = (ImageView) view.findViewById(R.id.recorder_surface_state);
		
		progressView = (ProgressView) view.findViewById(R.id.recorder_progress);
		cancelBtn = (Button) view.findViewById(R.id.recorder_cancel);
		cancelBtn.setOnClickListener(this);
		nextBtn = (Button) view.findViewById(R.id.recorder_next);
		nextBtn.setOnClickListener(this);
		//txtTimer = (TextView)findViewById(R.id.txtTimer);
		flashIcon = (Button)view.findViewById(R.id.recorder_flashlight);
		switchCameraIcon = (Button)view.findViewById(R.id.recorder_frontcamera);
	...
	public class AsyncStopRecording extends AsyncTask<Void,Integer,Void>{
		...
		protected void onPreExecute() {
			...
			//创建处理进度条
			creatingProgress= new Dialog(getActivity(),R.style.Dialog_loading_noDim);
		...
		private void getFirstCapture(byte[] data){
			...
			captureBitmapPath = Util.createImagePath(getActivity());
	...
	private void showCancellDialog(){
		Util.showDialog(getActivity(), "提示", "确定要放弃本视频吗？", 2, new Handler(){
			@Override
			public void dispatchMessage(Message msg) {
				if(msg.what == 1)
					videoTheEnd(false);
...
//	    }
	...
	private void handleSurfaceChanged()
	{
		...
		if(Build.VERSION.SDK_INT >  Build.VERSION_CODES.FROYO)
		{
			mCamera.setDisplayOrientation(Util.determineDisplayOrientation(getActivity(), defaultCameraId));
	...
	private void returnToCaller(boolean valid)
	{
		try{
			...
			if(valid){
				Intent intent = new Intent(getActivity(),FFmpegPreviewActivity.class);
	...
	private void registerVideo()
	{
		...
		try{
			uriVideoPath = getActivity().getContentResolver().insert(videoTable, Util.videoContentValues);
		} catch (Throwable e){
			uriVideoPath = null;
			strVideoPath = null;
			e.printStackTrace();
